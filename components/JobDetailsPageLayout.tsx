...
const JobDetailsPageLayout: React.FC<JobDetailsPageLayoutProps> = (props) => {
   ...
   const onClickOnShortList = () => {
      dispatch({ setState: { loading: true } });
      updateApplication(props.jobId, { type: "SHORTLIST", applicationIDs: Array.from(selectedApplicatoins) }).then(
         () => {
            setTimeout(() => {
         ...
         <div className="grid grid-rows-[auto] h-full md:grid md:grid-cols-[17.5rem_auto] w-full overflow-auto">
            <div className="md:flex flex-col w-full p-4 hidden">
               ...
               <JobDetailsSidebarItem
                  ...
                  badgeNumber={props.allApplications.filter((x) => x.status === "UNDER_REVIEW").length}
                  onClick={() => router.push(routes.company.jobDetails.applications(router.query.id as string))}
                  selected={props.selectedItem === 1}
               ></JobDetailsSidebarItem>
               <JobDetailsSidebarItem
                  text="Shortlisted"
                  badgeNumber={props.allApplications.filter((x) => x.status === "SHORTLISTED").length}
                  onClick={() => router.push(routes.company.jobDetails.applications(router.query.id as string))}
                  selected={props.selectedItem === 2}
               ></JobDetailsSidebarItem>
               <JobDetailsSidebarItem
                  text="Hired"
                  badgeNumber={props.allApplications.filter((x) => x.status === "UNDER_EVALUATION").length}
                  onClick={() => router.push(routes.company.jobDetails.applications(router.query.id as string))}
                  selected={props.selectedItem === 3}
               ></JobDetailsSidebarItem>
               <JobDetailsSidebarItem
                  text="Not interested"
                  badgeNumber={props.myApplications.filter((x) => x.status === "REJECTED").length}
                  onClick={() => router.push(routes.company.jobDetails.applications(router.query.id as string))}
               ...
               >
                  ...
                  >
                     ...
                     <div className="grid grid-cols-2 gap-3 md:flex whitespace-nowrap">
                        ...
                        <button className="btn btn-outlined btn-gray btn-sm" onClick={onClickOnShortList}>
                           Shortlist
                        </button>
